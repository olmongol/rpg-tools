        ...
        if self.__filein != "" and self.__filein != ():
            ...
            if self.__filein[-4:].lower() == "json":
                ...
                with open(self.__filein, 'r') as filecontent:
                    ...
                    dummy = self.__filein[:-4].split("/")[-2:]
                    pprint(dummy)  # rm.sldirs
                    pprint(self.spellist)
            else:
                msg = messageWindow()
...
#                                              ).grid(column = 1, row = 0, sticky = EW)
        self.__slData['sltype'] = StringVar()
        self.__slData['sltype'].set(rm.spellisttypes[self.lang][0])
        OptionMenu(self.window,
                   self.__slData['sltype'],
                   *rm.spellisttypes[self.lang],
                   command = self.__updSLData
                   ).grid(column = 1, row = 0, sticky = "EW")
        Label(master = self.window,
#              width = 15,
...
#              height = 60,
              ...
              ).grid(column = 4, columnspan = 2, rowspan = 2, row = 2, sticky = "NEWS")
        ...
        Entry(self.window,
              textvariable = self.__slData["type"]
              ).grid(column = 5, row = 5, sticky = "NEWS")
#        self.__slData['type'] = StringVar()
#        self.__slData['type'].set(rm.spellisttypes[self.lang][0])
#        OptionMenu(self.window,
#                   self.__slData['type'],
#                   *rm.spellisttypes[self.lang],
#                   command = self.__updSLData
#                   ).grid(column = 5, row = 5)
        ...
        Label(master = self.window,
              ...
              ).grid(column = 0, columnspan = 5, row = 6, sticky = "NEWS")

#*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
### sollte durch eine textbox ersetz werden
        self.__slData['descr'] = StringVar()
        Entry(master = self.window,
#              width = 200,
#              height = 100,
              textvariable = self.__slData['descr']
              ).grid(column = 0, columnspan = 5, row = 7, rowspan = 4, sticky = "NEWS")
